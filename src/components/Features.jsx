...
// Draw the current frame (should be at 0 seconds)
...
// Preload video and generate thumbnail when component mounts
useEffect(() => {
if (videoRef.current) {
...
const handleCanPlay = () => {
setIsLoaded(true);
// Ensure we're at the beginning for thumbnail generation
if (!thumbnailUrl) {
video.currentTime = 0;
}
};
...
const handleSeeked = () => {
// Generate thumbnail after seeking to 0
if (!thumbnailUrl && video.currentTime === 0) {
generateThumbnail(video);
}
};
...
<div 
className={`absolute inset-0 z-10 bg-cover bg-center transition-opacity duration-500 ${isPlaying && isLoaded ? "opacity-0" : "opacity-100"}`} 
style={{ 
backgroundImage: thumbnailUrl ? `url(${thumbnailUrl})` : `url(${video.thumbnail})`,
...
<video 
ref={videoRef} 
className={`w-full h-full object-cover transition-opacity duration-500 ${isPlaying && isLoaded ? "opacity-100" : "opacity-0"}`}
loop 
muted 
playsInline 
preload="metadata"
